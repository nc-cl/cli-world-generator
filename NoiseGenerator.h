        ...
        static float** _getOctave(float** noise, int octaveNumber, float lacunarity, int width, int height) {
            ...
            for (int i = 0; i < width; i++) {
                ...
                for (int j = 0; j < height; j++) {
                    ...
                    float bottom = NoiseGenerator::_lerp(noise[i0][j0], noise[i1][j0], hblend);
                    float top = NoiseGenerator::_lerp(noise[i0][j1], noise[i1][j1], hblend);
                    octave[i][j] = NoiseGenerator::_lerp(bottom, top, vblend);
                }
            }
